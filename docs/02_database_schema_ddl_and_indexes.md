# 02_database_schema_ddl_and_indexes.md — Consolidated PostgreSQL DDL, Constraints & Index Specifications

## 1. Overview & Database Design Principles
This document defines the complete PostgreSQL schema for Ledgify across all 22 core tables.
Every table enforces strict multi-tenant scoping via `business_id` (UUID), foreign key integrity, check constraints, and optimized indexing (B-Tree for tenant keys and foreign keys; GIN/GIST for Trigram & Phonetic similarity).

---

## 2. PostgreSQL Extensions & Custom Types

```sql
-- Enable necessary cryptographic and fuzzy matching extensions
...
3. Complete DDL for All 22 Tables
...
3.2 Master Hierarchy & Chart of Accounts
SQL
...
3.3 Transactions & Double-Entry Ledger Engine
SQL
-- 5. Vouchers Header
...
-- 6. Voucher Line Items (Debits & Credits)
CREATE TABLE public.voucher_line_items (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    business_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
    ...
    stock_item_id UUID, -- Optional foreign key to stock_items
    godown_id UUID,     -- Optional foreign key to godowns
    batch_id UUID,      -- Optional foreign key to stock_batches
...
3.5 GST Compliance, E-Invoicing & E-Way Bills
SQL
-- 8. E-Invoice Logs (FORM GST INV-01)
...
-- 10. GSTR Returns & Inward Supplies Management (IMS)
CREATE TABLE public.gstr_returns_ims (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    business_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
...
3.6 Inventory, Multi-Godown & Batch Management
SQL
-- 11. Stock Groups and Categories
CREATE TABLE public.stock_groups_categories (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    business_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
...
-- 13. Godowns / Multi-Location Warehouses
...
-- 14. Stock Batches & Expiry Tracking
...
3.7 Banking, Payroll & Direct Taxation
SQL
-- 15. Bank Accounts
...
CREATE TABLE public.bank_statements_brs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    business_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE public.employees_payroll (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    business_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
...
-- 18. TDS & TCS Entries
CREATE TABLE public.tds_tcs_entries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    business_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
    voucher_id UUID NOT NULL REFERENCES public.vouchers(id) ON DELETE CASCADE,
...
3.8 Monetization, Audit & DPDP Compliance
SQL
...
CREATE TABLE public.billing_webhook_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
4. Performance Indexing Blueprint
SQL
-- Standard B-Tree Indexes on RLS Tenant Filter (`business_id`)
CREATE INDEX idx_accounts_business_id ON public.accounts(business_id);
CREATE INDEX idx_vouchers_business_id ON public.vouchers(business_id);
CREATE INDEX idx_voucher_line_items_business_id ON public.voucher_line_items(business_id);
CREATE INDEX idx_fixed_assets_business_id ON public.fixed_assets(business_id);
CREATE INDEX idx_einvoice_logs_business_id ON public.einvoice_logs(business_id);
CREATE INDEX idx_eway_bills_business_id ON public.eway_bills(business_id);
CREATE INDEX idx_stock_items_business_id ON public.stock_items(business_id);
CREATE INDEX idx_stock_batches_business_id ON public.stock_batches(business_id);
CREATE INDEX idx_edit_logs_business_id ON public.edit_logs(business_id);

-- Foreign Key Performance Indexes
CREATE INDEX idx_voucher_line_items_voucher_id ON public.voucher_line_items(voucher_id);
CREATE INDEX idx_voucher_line_items_account_id ON public.voucher_line_items(account_id);
CREATE INDEX idx_vouchers_date ON public.vouchers(business_id, voucher_date);

-- GIST & GIN Trigram Indexes for 2-Stage Entity Search
CREATE INDEX trgm_idx_accounts_name_gist ON public.accounts USING GIST (name gist_trgm_ops(siglen=32));
CREATE INDEX trgm_idx_stock_items_name_gist ON public.stock_items USING GIST (name gist_trgm_ops(siglen=32));
CREATE INDEX trgm_idx_bank_statements_desc_gin ON public.bank_statements_brs USING GIN (description gin_trgm_ops);

-- GIN Indexes for Daitch-Mokotoff Phonetic Arrays
CREATE INDEX idx_accounts_phonetic_gin ON public.accounts USING GIN (daitch_mokotoff_code);
CREATE INDEX idx_stock_items_phonetic_gin ON public.stock_items USING GIN (daitch_mokotoff_code);
